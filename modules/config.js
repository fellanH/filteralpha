export const URL = {
  english:
    "https://zinrec.intervieweb.it/annunci.php?lang=en&LAC=erqole&d=miramis.com&k=603e695eadef987e4321536bd158a881&CodP=&format=json_en&utype=0",
  italian:
    "https://zinrec.intervieweb.it/annunci.php?lang=it&LAC=erqole&d=miramis.com&k=603e695eadef987e4321536bd158a881&CodP=&format=json_en&utype=0",
};

export const IMAGES = {
  miramis:
    "https://cdn.prod.website-files.com/671a1cb360117ed1b3e3d64a/67534780d64e7e95acc6b563_Miramis%20Icon.svg",
  torre:
    "https://cdn.prod.website-files.com/671a1cb360117ed1b3e3d64a/682ca0395ac35328bb8a61f7_logo%20tcp%20tondo.jpg",
  laroqqa:
    "https://cdn.prod.website-files.com/671a1cb360117ed1b3e3d64a/675348f19debe3c74c872d24_La%20Roqqa%20icon.svg",
};

export const ATTR = {
  careerContainer: `stormfors="container"`,
  jobList: `stormfors="job-list"`,
  filterList: `stormfors="filter-list"`,
  filter: {
    item: `filter="item"`,
    dropdown: `filter="dropdown"`,
    button: `filter="button"`,
    reset: `filter="reset"`,
  },
  job: {
    item: `stormfors="item"`,
    brand: `job-data="brand"`,
    title: `job-data="title"`,
    location: `job-data="location"`,
    type: `job-data="type"`,
    department: `job-data="department"`,
    date: `job-data="date"`,
  },
  drawer: {
    trigger: `stormfors="drawer-trigger"`,
    content: `stormfors="drawer-content"`,
    close: `stormfors="drawer-close"`,
  },
};
